...
<head>
  ...
  <style>
    *,*::before,*::after{box-sizing:border-box;}
    ...
    .rbar{max-width:1040px;margin:.7rem auto 0;padding:0 1rem;display:flex;align-items:center;justify-content:space-between;gap:.5rem;flex-wrap:wrap;}
    .rcnt{font-size:.84rem;color:#4a5568;}
    .rcnt strong{color:#2d3748;}
    .rtools{display:flex;gap:.4rem;align-items:center;}
    ...
    .ghost{background:#fff;color:#4a5568;border:1px solid #cbd5e0;}
    .ghost:hover{background:#f7fafc;}
    .f-chipbar{max-width:1040px;margin:.4rem auto 0;padding:0 1rem;display:flex;flex-wrap:wrap;gap:.35rem;align-items:center;}
    .chip{font-size:.72rem;color:#2b6cb0;background:#ebf8ff;border:1px solid #bee3f8;border-radius:999px;padding:.13rem .55rem;}
    .wrap{max-width:1040px;margin:.5rem auto 0;padding:0 1rem 3rem;}
    ...
    @media(max-width:640px){.hd,.filters,.tabs{padding-left:1rem;padding-right:1rem;}.wl-grid{grid-template-columns:1fr;}.run-err{margin-left:1rem;margin-right:1rem;}.rbar{align-items:stretch;}.rtools{width:100%;justify-content:flex-end;}}
  </style>
</head>
<body>
<div class="hd">
...
<div id="pane-db" style="display:none">
  <div class="filters">
    <div class="frow">
      ...
      <label class="dr" style="margin:0"><span>表示</span>
        <select class="ss" id="pageSize" onchange="af()">
      ...
      <button class="qb" onclick="resetFilters()">フィルタ解除</button>
    </div>
  ...
  <div class="f-chipbar" id="chipBar"></div>
  <div class="rbar">
    <span class="rcnt" id="rcnt"></span>
    <div class="rtools">
      <button class="ebtn ghost" onclick="window.scrollTo({top:0,behavior:'smooth'})">先頭へ</button>
...
function uiEls(){
  return {
    ...
    kwOnly: document.getElementById('kwOnly'),
    df: document.getElementById('df'),
    dt: document.getElementById('dt'),
    sb: document.getElementById('sb'),
    dbTheme: document.getElementById('dbTheme'),
    wlTheme: document.getElementById('wlTheme'),
...
function saveUiState(){
  try{
    var x=uiEls();
    var p=Array.from(document.querySelectorAll('.sci:checked')).map(function(c){return c.value;});
    var st={q:x.q&&x.q.value||'',kwOnly:!!(x.kwOnly&&x.kwOnly.checked),df:x.df&&x.df.value||'',dt:x.dt&&x.dt.value||'',sb:x.sb&&x.sb.value||'newest',dbTheme:x.dbTheme&&x.dbTheme.value||'',wlTheme:x.wlTheme&&x.wlTheme.value||'',pageSize:x.pageSize&&x.pageSize.value||'30',pickedSources:p};
    localStorage.setItem('lawAlertUiState',JSON.stringify(st));
  }catch(_){}
}
function loadUiState(){
  try{
    var raw=localStorage.getItem('lawAlertUiState');
    if(!raw)return null;
    return JSON.parse(raw);
  }catch(_){return null;}
}
function applyUiState(){
  var st=loadUiState(); if(!st)return;
  var x=uiEls();
  if(x.q)x.q.value=st.q||'';
  if(x.kwOnly)x.kwOnly.checked=!!st.kwOnly;
  if(x.df)x.df.value=st.df||'';
  if(x.dt)x.dt.value=st.dt||'';
  if(x.sb&&st.sb)x.sb.value=st.sb;
  if(x.dbTheme&&st.dbTheme)x.dbTheme.value=st.dbTheme;
  if(x.wlTheme&&st.wlTheme)x.wlTheme.value=st.wlTheme;
  if(x.pageSize&&st.pageSize)x.pageSize.value=String(st.pageSize);
  if(Array.isArray(st.pickedSources)&&st.pickedSources.length){
    var pick=new Set(st.pickedSources);
    document.querySelectorAll('.sci').forEach(function(c){c.checked=pick.has(c.value);});
  }
}
function updateChipBar(){
  var chip=document.getElementById('chipBar'); if(!chip)return;
  var x=uiEls();
  var arr=[];
  if(x.q&&x.q.value.trim())arr.push('検索: '+x.q.value.trim());
  if(x.kwOnly&&x.kwOnly.checked)arr.push('キーワード一致のみ');
  if(x.dbTheme&&x.dbTheme.value)arr.push('テーマ: '+x.dbTheme.value);
  if(x.df&&x.df.value)arr.push('開始: '+x.df.value);
  if(x.dt&&x.dt.value)arr.push('終了: '+x.dt.value);
  var total=document.querySelectorAll('.sci').length;
  var selected=document.querySelectorAll('.sci:checked').length;
  if(total && selected<total)arr.push('ソース: '+selected+'/'+total);
  chip.innerHTML=arr.length?arr.map(function(t){return '<span class="chip">'+e(t)+'</span>';}).join(''):'';
}
function resetFilters(){
  var x=uiEls();
  if(x.q)x.q.value='';
  if(x.kwOnly)x.kwOnly.checked=false;
  if(x.df)x.df.value='';
  if(x.dt)x.dt.value='';
  if(x.sb)x.sb.value='newest';
  if(x.dbTheme)x.dbTheme.value='';
  if(x.wlTheme)x.wlTheme.value='';
  if(x.pageSize)x.pageSize.value='30';
  document.querySelectorAll('.sci').forEach(function(c){c.checked=true;});
  sp(null); buildWL(); af();
}
...
function af(){
  var pz=parseInt((document.getElementById('pageSize')||{}).value||'30',10);
  PG=(Number.isFinite(pz)&&pz>0)?pz:30;
